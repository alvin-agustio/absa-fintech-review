...
def main():
    parser = argparse.ArgumentParser(description="Train IndoBERT + LoRA for ABSA")
    ...
    output_dir = args.output_dir or str(MODELS_DIR / "lora")
    ...
    print(f"\n[LORA] LoRA Config: r={args.lora_r}, alpha={args.lora_alpha}, dropout={args.lora_dropout}")
    ...
    preds_df = test_df[["review_id", "aspect", "review_text", "label"]].copy()
    preds_df["pred_label"] = [ID2LABEL[int(x)] for x in y_pred]
    preds_df["is_error"] = preds_df["label"] != preds_df["pred_label"]
    ...
    print(f"\n[LORA] Training complete in {train_time:.1f}s")
